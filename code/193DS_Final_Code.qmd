...
krat_subset_summary
>>>>>>> b812c364edbe2accc4fb86e50636b9327c27eca7
```

```{r}
ggplot(data = krat_subset) + stat_qq(aes(sample = seeds)) + stat_qq_line(aes(sample = seeds), color = "red") + facet_wrap(~ loc)
```

```{r}
<<<<<<< HEAD
mass.var_krat <- var.test(seeds, loc, data = krat_subset, conf.level = 0.975)
=======
...
mass_var <- var.test(stem_dry_mass ~ watershed, data = maples_2003, conf.level = 0.975)
mass_var 
...
=======
>>>>>>> 0b778e4bf1d9e846e9849c81bdea918870aa1437
